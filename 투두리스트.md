...
- [ ] smoke test 환경 의존성 해결 — `egfr_pipeline.residue_utils` 모듈 부재로 10개 테스트 파일 collection error 발생
...
- [ ] P2Rank 통합 — 현재 fpocket만 사용. Phase 2 pocket proposal에 P2Rank 파서 구현됨(`pocket_proposal.py`)이나 실제 통합 미완료. Druggability 축(A2) 판별력 개선 예상
- [ ] Orientation filter ambiguous band 임계값(+-0.15) 경험적 검증 — 파일럿 결과 기반 조정 필요 (`docs/phase1_notes.md`)
- [ ] Sheet 12 역할 규명 — 기능적으로 필수이나 직접 접촉 vs 구조적 지지 미확정 (`docs/phase1_notes.md`)
