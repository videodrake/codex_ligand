...
# CHARMM -> canonical residue name mapping (MD outputs use HSD/HSE/HSP etc.)
_RESNAME_NORMALIZE = {
...
def normalize_residue_id(residue_id: str) -> str:
    """Normalize a residue id like 'X:HSD850' -> '850'.

    For cross-receptor comparison we strip chain ID and normalize the
    residue name, then return 'RESNAME:RESNUM' so that the same physical
    residue on different chains/receptors maps to the same key.
    """
    # Format: "CHAIN:RESNAMERESNUM" e.g. "A:MET971" or "X:HSD850"
    if ":" in residue_id:
        residue_id = residue_id.split(":", 1)[1]
    # Separate name from number: find where digits start
    i = 0
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare pockets across receptor states (Task Group 5).",
    )
    parser.add_argument("--config", required=True, help="Project YAML/JSON config path")
    parser.add_argument("--pocket-table", default=None, help="Pocket table CSV (default: <project_root>/vina_pocket_table.csv)")
    parser.add_argument("--drug-map", default=None, help="Drug pocket map CSV (default: <project_root>/vina_drug_pocket_map.csv)")
    parser.add_argument(
        "--centroid-cutoff", type=float, default=15.0,
        help="Max centroid distance to include in comparison (A). 0 = all pairs. (default: 15.0)",
    )
    parser.add_argument("--output", default=None, help="Output CSV path (default: <project_root>/vina_pocket_comparison.csv)")
    return parser.parse_args()


def main():
    args = parse_args()
    out = compare_from_config(
        args.config,
        pocket_table_path=args.pocket_table,
        drug_map_path=args.drug_map,
        centroid_cutoff=args.centroid_cutoff,
        output_path=args.output,
    )
    print(f"[OK] Wrote pocket comparison: {out}")
