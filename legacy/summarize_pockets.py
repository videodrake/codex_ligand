...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize clustered Vina poses into pocket-level outputs.")
...
def main():
    args = parse_args()
    pocket_csv, drug_csv = summarize_from_config(args.config, args.pose_table)
    print(f"[OK] Wrote pocket table: {pocket_csv}")
    print(f"[OK] Wrote drug pocket map: {drug_csv}")
