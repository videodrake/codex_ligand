...
def parse_args():
    parser = argparse.ArgumentParser(description="Parse Vina PDBQT outputs into a pose-level table.")
    parser.add_argument("--config", required=True, help="Project YAML/JSON config path")
    parser.add_argument("--output", default=None, help="Output CSV path (default: <project_root>/vina_pose_table.csv)")
    return parser.parse_args()


def main():
    args = parse_args()
    output_csv = build_pose_table_from_config(args.config, args.output)
    print(f"[OK] Wrote pose table: {output_csv}")
