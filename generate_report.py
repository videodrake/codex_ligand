...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate project summary report (Task Group 7).",
    )
...
def main():
    args = parse_args()
    report_path, combined_csv = generate_report(args.config, args.output_dir)
    print(f"[OK] Report: {report_path}")
    print(f"[OK] Combined residue evidence: {combined_csv}")
