...
## ⚠️ 이 환경과 HPC는 완전히 분리되어 있다

이 환경(Claude Code)은 코드 수정 전용이다. HPC 서버와 네트워크가 연결되어 있지 않다.

이 환경에서 가능한 것:
- 코드 수정, 테스트 작성, 문서 작성, PBS 스크립트 생성
- pytest -m smoke (기본 의존성 범위 내)

이 환경에서 불가능한 것:
- output/ 결과 접근 — 프로덕션 결과는 HPC에만 존재하고 이 환경에 없다
- PyRosetta, Vina, LightDock, fpocket 실행
- pytest -m full
- HPC 서버 접속

사람이 중간 다리 역할을 한다:
1. 이 환경에서 코드를 수정하고 HPC 실행 명령어(qsub 등)를 안내한다
2. 사용자가 HPC에서 실행한다
3. 결과 분석이 필요하면 사용자가 결과 파일을 이 환경에 전달해준다
4. 전달받은 파일로 분석을 수행한다

이것을 항상 기억한다:
- output/ 디렉토리를 읽으려 하지 않는다. 없는 게 정상이다.
- 결과가 필요한 요청을 받으면 "HPC 결과를 전달해주세요"라고 요청한다.
- output/ 경로를 참조하는 코드를 짤 때는 파일 미존재를 정상 상태로 처리한다.
- "결과를 분석해줘" 요청에 결과 파일이 없으면, 분석 스크립트를 먼저 작성하고
  사용자에게 HPC에서 실행할 명령어를 안내한다.

이 섹션을 절대 규칙보다 위에 두는 이유:
세션마다 가장 먼저 읽히는 위치에 있어야 output/ 접근 시도를 원천 차단한다.
...
5. **CSV 스키마 단일 출처**: 모든 CSV 컬럼 정의는 `egfr_pipeline/schemas.py`에 있다. 컬럼 추가/제거/순서 변경은 schemas.py에서 한다 — producer(verdict, parse_poses, pyrosetta_extract 등)와 validate.py가 같은 상수를 import한다. 인라인 컬럼 리스트(`fieldnames=["..."]`)를 다시 도입하지 않는다.
