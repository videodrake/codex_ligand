...
def run_m3_ligand_qc(
    ...
    for public_id in PUBLIC_COMPOUND_IDS:
        ...
        if not exists:
            ...
            _append_qc(qc_rows, public_id, "FORMAT_ALLOWED", "file_qc", "PASS" if selected.suffix.lower() in ALLOWED_EXTENSIONS else "FAIL", "format allowed" if selected.suffix.lower() in ALLOWED_EXTENSIONS else "unsupported format", "Use one of .sdf/.mol2/.pdb/.smi/.smiles/.pdbqt; supplied PDBQT is validated in M3-T2.", selected.suffix.lower() not in ALLOWED_EXTENSIONS)
