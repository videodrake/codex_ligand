...
def generate_report(
    ...
    # AC-5.5: Disclaimer at top of report
    report_lines.append("=" * 72)
    report_lines.append("DISCLAIMER")
    report_lines.append(
        "This report contains computational predictions generated by automated\n"
        "molecular docking and scoring pipelines. All results are hypotheses\n"
        "requiring experimental validation. Binding site predictions, affinity\n"
        "estimates, and PPI perturbation scores should not be interpreted as\n"
        "confirmed biological effects. See methodology_limitations.md for\n"
        "known limitations of the computational methods used."
    )
    report_lines.append("=" * 72)
    report_lines.append("")
    ...
    # Section 4.7: Allosteric candidate pockets (AC-5.3)
    allosteric_pockets = [r for r in verdict_rows
                          if str(r.get("allosteric_candidate", "")).lower() in ("true", "1")]
    if allosteric_pockets:
        report_lines.append(section_header("4.7 Allosteric Candidate Pockets", 2))
        report_lines.append(
            "  Pockets with high Vina quality (>=35) but low PPI proximity (<=5).\n"
            "  These may disrupt MYO1D binding through conformational changes\n"
            "  rather than direct steric competition. Experimental validation\n"
            "  (e.g., MD simulation) is needed to confirm allosteric mechanism.\n"
        )
        report_lines.append(f"  {'Receptor':<20} {'Pocket':<7} {'Verdict':<10} "
                            f"{'Vina':>6} {'PPI':>6} {'Total':>6}")
        report_lines.append(f"  {'--------':<20} {'------':<7} {'-------':<10} "
                            f"{'----':>6} {'---':>6} {'-----':>6}")
        for r in allosteric_pockets:
            report_lines.append(
                f"  {r.get('receptor_id',''):<20} {r.get('pocket_id',''):<7} "
                f"{r.get('verdict',''):<10} "
                f"{r.get('vina_quality_score',''):>6} "
                f"{r.get('ppi_proximity_score',''):>6} "
                f"{r.get('confidence_score',''):>6}"
            )
        report_lines.append("")
    else:
        report_lines.append(section_header("4.7 Allosteric Candidate Pockets", 2))
        report_lines.append("  No allosteric candidates identified "
                            "(no pocket with Vina >= 35 AND PPI <= 5).\n")
