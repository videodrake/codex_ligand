...
The **extended beta-meander (~residue 955–1006)** is currently the preferred primary input for interface mapping.
The earlier truncated construct (962–1006) produced a VAL962 N-terminal artifact and must be replaced.
...
The exact residue range depends on the numbering system (see PRD v2 for detailed numbering caution). The active kinase core spans approximately UniProt 696–979 (excluding C-terminal tail); 3GT8 PDB numbering differs by an offset of +24.
...
- Extended beta-meander (~955–1006) preparation
...
Additionally, MD validation (100–200 ns) of the top 1–2 cluster representatives is strongly recommended as a Phase 1→2 gate, though it is not included in the Phase 1 core task list. If MD validation reveals that the top patch is dynamically unstable, Phase 2 pocket proposal would be premature.
...
- **that the truncated beta-meander (962–1006) is still the active partner input (extended ~955–1006 is now required),**
