...
| Contact distances | `vina_contact_distances.csv` | `output/egfr_myo1d_vina/vina_contact_distances.csv` | Long-form per-pose contact distances (residue_id, min_distance_A) | No direct next step; supports detailed contact analysis | `P3` |
| Residue occupancy | `vina_pocket_residue_occupancy.csv` | `output/egfr_myo1d_vina/vina_pocket_residue_occupancy.csv` | Per-pocket residue occupancy and hotspot flags | No direct next step; supports residue-level pocket characterization | `P2` |
| Clustering merge log | `vina_clustering_merge_log.csv` | `output/egfr_myo1d_vina/vina_clustering_merge_log.csv` | Pocket merge provenance with merge reasons | No; provenance and reproducibility | `P3` |
| Clustering parameters | `vina_clustering_parameters.json` | `output/egfr_myo1d_vina/vina_clustering_parameters.json` | Clustering parameter snapshot | No; reproducibility record | `P3` |
...
| Cluster summary | `output/phase1_ppi/<state>/ppi_cluster_summary.csv` | Cluster-level summary with centroid coordinates (centroid_x/y/z), centroid_spread_A, dSASA decomposition, and energy distribution statistics (std/min/max) | No direct phase handoff; analysis support | `P3` |
