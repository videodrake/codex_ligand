...
- Some files directly under `output/egfr_myo1d_vina/` are pointer stub files, not full payload files. The canonical payload usually lives under `vina/`, `ppi/`, or `results/`.
...
|------|------|------|------|------|------|
| Vina poses | `vina_pose_table.csv` | `output/egfr_myo1d_vina/vina/vina_pose_table.csv` | Pose-level table with receptor, ligand, pose rank, affinity, centroid, raw pose file path, pocket ID, and contact residues | Yes -> supports pocket summarization, comparison, report, and validation | `P2` |
| Vina pockets | `vina_pocket_table.csv` | `output/egfr_myo1d_vina/vina/vina_pocket_table.csv` | Pocket-level aggregation of Vina poses within each receptor | Yes -> consumed by verdict, report, and validate | `P1` |
| Ligand-pocket map | `vina_drug_pocket_map.csv` | `output/egfr_myo1d_vina/vina/vina_drug_pocket_map.csv` | Ligand-to-pocket summary showing dominant and multimodal mappings | Yes -> consumed by report and interpretation | `P2` |
| Cross-state pocket comparison | `vina_pocket_comparison.csv` | `output/egfr_myo1d_vina/vina/vina_pocket_comparison.csv` | Cross-receptor pocket comparison table | Yes -> consumed by verdict, report, and validate | `P2` |
| Pocket bootstrap | `vina_pocket_bootstrap.csv` | `output/egfr_myo1d_vina/vina/vina_pocket_bootstrap.csv` | Optional resampling-based support for pocket stability | Optional downstream support only | `P3` |
| PPI residue summary | `ppi_pyrosetta_residues.csv` | `output/egfr_myo1d_vina/ppi/ppi_pyrosetta_residues.csv` | Project-root PPI residue extraction table from the older baseline layout | Yes -> contributes to verdict and report in the routine baseline | `P2` |
| PPI run summary | `ppi_pyrosetta_summary.csv` | `output/egfr_myo1d_vina/ppi/ppi_pyrosetta_summary.csv` | Aggregated summary of PyRosetta-derived PPI results in the older layout | Supporting only | `P3` |
| Cross-method agreement | `cross_method_agreement.csv` | `output/egfr_myo1d_vina/results/cross_method_agreement.csv` | Main Vina-vs-PPI overlap and proximity table for the routine baseline | Yes -> used by report and validate; presentation-facing | `P1` |
| Site verdicts | `valid_sites.csv` | `output/egfr_myo1d_vina/results/valid_sites.csv` | Final rule-based site judgment table from `egfr_pipeline/verdict.py` | Yes -> used by report and validate; often the first baseline judgment table | `P1` |
| Consensus site map | `vina_consensus_sites.csv` | `output/egfr_myo1d_vina/results/vina_consensus_sites.csv` | Consensus pocket/site grouping across receptor states | No direct next step in baseline; review and presentation support | `P2` |
| Combined residue evidence | `combined_residue_evidence.csv` | `output/egfr_myo1d_vina/results/combined_residue_evidence.csv` | Residue-level integrated evidence view used for report support | No direct next step; interpretation support | `P2` |
| Top-level report | `project_report.txt` | `output/egfr_myo1d_vina/results/project_report.txt` | Human-readable summary report for the current baseline | No; presentation/report endpoint | `P1` |
...
|------|------|
| What does the current routine baseline think are the important sites? | `output/egfr_myo1d_vina/results/valid_sites.csv`, `output/egfr_myo1d_vina/results/cross_method_agreement.csv`, `output/egfr_myo1d_vina/results/project_report.txt` |
